...
import { mercuryPalette, sampleMercury, type MercuryBiomeId } from './mercury-terrain';
...
const palette = { ocean: '#2675a6', forest: '#629441', desert: '#d3b879', snow: '#d7e6e5', mountain: '#8f9181', ...marsPalette, ...venusPalette, ...mercuryPalette };
...
export async function createEarth(container: HTMLElement, signal: AbortSignal, callbacks: Callbacks, planetId: 'earth' | 'mercury' | 'mars' | 'saturn' | 'venus' | 'jupiter' = 'earth'): Promise<EarthAPI> {
  ...
  const isRocky = isMars || isVenus || isMercury;
  const sampleTerrain = isMercury ? sampleMercury : isVenus ? sampleVenus : sampleMars;
  ...
  scene.add(planet);
  const fill = new THREE.HemisphereLight(isMercury ? '#fff1cf' : isVenus ? '#ffebbf' : isMars ? '#ffe0c4' : '#c8e5ff', isMercury ? '#1d1b19' : isVenus ? '#352719' : isMars ? '#351b18' : '#132b36', 2.35);
  scene.add(fill);
  const sun = new THREE.DirectionalLight('#fff3d3', 3.7);
  sun.position.set(-5, 7, 6); scene.add(sun);
  const rim = new THREE.DirectionalLight(isMercury ? '#d9b97f' : isVenus ? '#e8c576' : isMars ? '#e89972' : '#74bafa', 2.0);
  ...
  if (isMercury) {
    cloudCenters.length = 0;
    for (let lon = -165; lon <= 165; lon += 55) cloudCenters.push([0, lon]);
  }
  ...
  const cloudMaterial = new THREE.MeshStandardMaterial({ roughness: 1, color: isMercury ? '#e2c58d' : isVenus ? '#edcf86' : isMars ? '#cc9873' : '#edf4ef', transparent: isRocky, opacity: isMercury ? 0.12 : isVenus ? 0.55 : isMars ? 0.22 : 1, depthWrite: !isRocky });
  ...
  const orbitMaterial = new THREE.LineDashedMaterial({ color: isMercury ? '#8f8574' : isVenus ? '#b19a60' : isMars ? '#99705b' : '#55756e', transparent: true, opacity: .24, dashSize: .07, gapSize: .08 });

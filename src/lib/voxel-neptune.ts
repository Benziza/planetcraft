...
import {
  ...
  neptuneFitDistance,
  type NeptuneBiomeId,
} from './neptune-terrain';

type Callbacks = {
  onSelect: (biome: NeptuneBiomeId, lat: number, lon: number) => void;
  onInteract: () => void;
};
...
export async function createNeptune(
  ...
  planet.rotation.z = -NEPTUNE_SHAPE.axialTilt;
  scene.add(planet);
  const fill = new THREE.HemisphereLight('#bde8ff', '#081a4a', 2.25);
  const sun = new THREE.DirectionalLight('#e8f6ff', 3.45);
  sun.position.set(-6, 8, 7);
  const rim = new THREE.DirectionalLight('#236cff', 2.15);
  ...
  for (let x = 0; x < 16; x++)
    for (let y = 0; y < 16; y++) {
      const value = Math.round(215 + noise(x, y, 8) * 38);
  ...
  cells.forEach((cell, index) => {
    ...
    dummy.scale.setScalar(NEPTUNE_SHAPE.voxelSize);
    dummy.updateMatrix();
    body.setMatrixAt(index, dummy.matrix);
    color
      .set(cell.color)
      .multiplyScalar(0.88 + noise(cell.x, cell.y, cell.z) * 0.17);
    body.setColorAt(index, color);
  });
  ringCells.forEach((cell, index) => {
    dummy.position.set(cell.x, 0, cell.z);
    dummy.scale.set(
      NEPTUNE_RINGS.voxelSize,
      NEPTUNE_RINGS.thickness,
      NEPTUNE_RINGS.voxelSize,
    );
    dummy.updateMatrix();
    ...
    color
      .set(cell.color)
      .multiplyScalar(0.72 + noise(cell.x, 0, cell.z) * 0.25);
  ...
  haze.scale.set(
    NEPTUNE_SHAPE.equatorialRadius + 0.06,
    NEPTUNE_SHAPE.polarRadius + 0.06,
    NEPTUNE_SHAPE.equatorialRadius + 0.06,
  );
  planet.add(haze);
  ...
  function resize() {
    ...
    const newFitDistance = neptuneFitDistance(camera.aspect, camera.fov);
    ...
    controls.minDistance = Math.max(6.2, fitDistance * 0.48);

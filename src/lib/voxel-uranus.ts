...
type Callbacks = {
  onSelect: (biome: UranusBiomeId, lat: number, lon: number) => void;
  onInteract: () => void;
};
...
export async function createUranus(
  ...
  const ringCells = buildUranusRings();
  const scene = new THREE.Scene();
  const camera = new THREE.PerspectiveCamera(37, 1, 0.1, 600);
  const initialDirection = new THREE.Vector3(0.72, 0.18, 1).normalize();
  let fitDistance = uranusFitDistance(1);
  ...
  // Uranus rotates almost on its side, and its rings share that tipped axis.
  const planet = new THREE.Group();
  planet.rotation.z = -URANUS_SHAPE.axialTilt;
  scene.add(planet);
  const fill = new THREE.HemisphereLight('#d9ffff', '#173943', 2.15);
  const sun = new THREE.DirectionalLight('#f1ffff', 3.25);
  sun.position.set(-6, 8, 7);
  const rim = new THREE.DirectionalLight('#5bd3e0', 1.85);
  ...
  for (let x = 0; x < 16; x++)
    for (let y = 0; y < 16; y++) {
      const value = Math.round(220 + noise(x, y, 4) * 34);
  ...
  cells.forEach((cell, index) => {
    ...
    dummy.scale.setScalar(URANUS_SHAPE.voxelSize);
    dummy.updateMatrix();
    body.setMatrixAt(index, dummy.matrix);
    color
      .set(cell.color)
      .multiplyScalar(0.9 + noise(cell.x, cell.y, cell.z) * 0.15);
    body.setColorAt(index, color);
  });
  ringCells.forEach((cell, index) => {
    dummy.position.set(cell.x, 0, cell.z);
    dummy.scale.set(
      URANUS_RINGS.voxelSize,
      URANUS_RINGS.thickness,
      URANUS_RINGS.voxelSize,
    );
    dummy.updateMatrix();
    ...
    color
      .set(cell.color)
      .multiplyScalar(0.82 + noise(cell.x, 0, cell.z) * 0.22);
  ...
  const hazeGeometry = new THREE.SphereGeometry(1, 48, 32);
  ...
  haze.scale.set(
    URANUS_SHAPE.equatorialRadius + 0.06,
    URANUS_SHAPE.polarRadius + 0.06,
    URANUS_SHAPE.equatorialRadius + 0.06,
  );
  planet.add(haze);
  ...
  function resize() {
    ...
    const newFitDistance = uranusFitDistance(camera.aspect, camera.fov);

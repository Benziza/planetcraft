...
import {
  ...
  jupiterFitDistance,
  type JupiterBiomeId,
} from './jupiter-terrain';

type Callbacks = {
  onSelect: (biome: JupiterBiomeId, lat: number, lon: number) => void;
  onInteract: () => void;
};
...
export async function createJupiter(
  ...
  const initialDirection = new THREE.Vector3(0, 0.12, 1).normalize();
  let fitDistance = jupiterFitDistance(1);
  ...
  // Keep the cloud features and atmosphere aligned with the spin axis.
  const planet = new THREE.Group();
  planet.rotation.z = -JUPITER_SHAPE.axialTilt;
  scene.add(planet);
  ...
  cells.forEach((cell, index) => {
    ...
    dummy.scale.setScalar(JUPITER_SHAPE.voxelSize);
    dummy.updateMatrix();
  ...
  haze.scale.set(
    JUPITER_SHAPE.equatorialRadius + 0.07,
    JUPITER_SHAPE.polarRadius + 0.07,
    JUPITER_SHAPE.equatorialRadius + 0.07,
  );
  planet.add(haze);
  ...
  function resize() {
    ...
    const newFitDistance = jupiterFitDistance(camera.aspect, camera.fov);
  ...
  const up = (event: PointerEvent) => {
    ...
    const hit = raycaster.intersectObjects([body], false)[0];
